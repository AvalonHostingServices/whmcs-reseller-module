                    ...
                    if (inputUsername.length) {
                        ...
                        if (tr.is('tr')) {
                            ...
                            if (labelTd.length) {
                                ...
                                if (txt !== 'API Endpoint (enter only if provided)') {
                ...
                if (selectedType === 'products_reseller_server') {
                    ...
                    if (inputUsername.length) {
                        ...
                        if (tr.is('tr')) {
                            var labelTd = tr.children('td.fieldlabel');
                            if (labelTd.length) {
                                labelTd.text('API Endpoint (enter only if provided)');
                            }
                        }
                    }
                    ...
                    var addLabel = $('label[for="addUsername"]');
                    if (addLabel.length) {
                        addLabel.text('API Endpoint (enter only if provided)');
                    }
...
add_hook('AdminAreaHeadOutput', 1, function($vars) {
    ...
    if($vars['filename'] == 'configproducts' && !isset($_GET['id'])) {
        $prsHooks = __DIR__ . '/../../../includes/hooks/prs_hooks.php';
        if(file_exists($prsHooks)) {
            unlink($prsHooks);
        }
        return <<<'HTML'
        <style>
            ...
            #prs-products-table td:nth-child(10) {
                min-width: 200px;
                max-width: 300px;
                vertical-align: top;
            }
            #prs-products-table th:nth-child(10) {
                min-width: 200px;
            }
        ...
        (function($) {
            ...
            function buildConfigOptionsHtml(configOptionGroups, marginValue, marginType) {
                if (!configOptionGroups || configOptionGroups.length === 0) {
                ...
                var optionTypeLabels = {1:'Dropdown',2:'Radio',3:'Checkbox',4:'Quantity',5:'Text',6:'Textarea'};

                var html = '';

                $.each(configOptionGroups, function(gi, group) {
                    html += '<div style="margin-bottom:8px;">'
                          + '<div style="font-weight:600; color:#1A4D80; font-size:12px; margin-bottom:3px;">'
                          + escapeHtml(group.name)
                          + '</div>';

                    $.each(group.options || [], function(oi, opt) {
                        var typeLabel = optionTypeLabels[opt.optiontype] || ('Type ' + opt.optiontype);
                        html += '<div style="margin-left:8px; margin-bottom:4px;">'
                              ...
                              + '</span>'
                              + '<span style="font-size:10px; color:#888; margin-left:4px;">(' + typeLabel + ')</span>';

                        if (opt.optiontype == 4) {
                            // Quantity — no sub-options, no per-sub pricing
                            html += '<div style="font-size:10px; color:#aaa; margin-left:10px;">Quantity field</div>';
                        } else {
                            $.each(opt.suboptions || [], function(si, sub) {
                                var hasPricing = sub.pricing && sub.pricing.length > 0;
                                var pricingHtml = '';

                                if (hasPricing) {
                                    $.each(sub.pricing, function(ci, cp) {
                                        var currency = cp.currency_code || '';
                                        var p = cp.pricing || {};
                                        var parts = [];

                                        $.each(cycles, function(idx, cycle) {
                                            var price = parseFloat(p[cycle] !== undefined ? p[cycle] : -1);
                                            var setup = parseFloat(p[setupKeys[idx]] !== undefined ? p[setupKeys[idx]] : 0);
                                            if (price < 0) return;
                                            ...
                                            var part = '<span style="white-space:nowrap;">'
                                                      + '<b>' + cycleLabels[cycle] + ':</b> '
                                                      + currency + ' ' + finalPrice.toFixed(2);
                                            if (finalSetup > 0) {
                                                part += ' <span style="color:#aaa;">(+' + currency + ' ' + finalSetup.toFixed(2) + ')</span>';
                                            }
                                            part += '</span>';
                                            parts.push(part);
                                        });
                                        ...
                                        if (parts.length > 0) {
                                            pricingHtml += '<span style="color:#888; font-size:10px;">'
                                                         + parts.join(' ')
                                                         + '</span>';
                                        }
                                    });
                                ...
                                html += '<div style="margin-left:10px; font-size:11px; padding:1px 0;">'
                                      + '<span style="color:#333;">&#8250; ' + escapeHtml(sub.optionname) + '</span>';
                                if (pricingHtml) {
                                    html += '<div style="margin-left:8px; line-height:1.5;">' + pricingHtml + '</div>';
                                } else {
                                    html += '<span style="color:#aaa; font-size:10px; margin-left:6px;">Free</span>';
                                }
                                html += '</div>';
                            });
                        }
                ...
                return html || '<span style="color:#aaa; font-size:12px;">None</span>';
            }
            ...
            function buildModal() {
                var html = '<div id="prs-import-modal-overlay" style="display:none;">'
                    ...
                    + '</div>'
                    + '<div style="max-height:auto; overflow-y:auto; border:1px solid #e0e5ea; border-radius:4px;">'
            ...
            function renderProducts(products) {
                ...
                $.each(products, function(i, p) {
                    ...
                    var configOptionsHtml = buildConfigOptionsHtml(p.config_option_groups || [], 0, 'percentage');
                    var row = '<tr data-index="' + i + '">'
                        ...
                        + '<td class="prs-config-options-cell" data-index="' + i + '">' + configOptionsHtml + '</td>'
                        + '</tr>';
                    $tbody.append(row);
                });
            ...
            function recalcFinalPrice(idx) {
                ...
                var configHtml = buildConfigOptionsHtml(p.config_option_groups || [], marginValue, marginType);
                $('.prs-config-options-cell[data-index="' + idx + '"]').html(configHtml);
            }
            ...
            function doImportSync() {
                ...
                $('.prs-product-check:checked').each(function() {
                    ...
                    selectedProducts.push({
                        ...
                        config_option_groups: p.config_option_groups || []
                    });
                });
